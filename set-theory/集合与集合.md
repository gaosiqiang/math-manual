...
## 8.4、差集运算
定理：设$I$为运算过程中的全集(基本集)，$A$是$I$的子集，则$I \setminus A$为A的余集或补集，记作$A^{c}$，公式$\complement_{I}A=I \setminus A=A^{c}$；

例子：
设实数集R，数集$A=\{x | 0<x \leqslant 1 \}$的余集为$A^{c}=\{x | x \geqslant 0 \lor x > 1 \}=R \setminus A$；
...
18、$(A \cup B)^{c}=A^{c} \cap B^{c}$，$(A \cap B)^{c}=A^{c} \cup B^{c}$；
